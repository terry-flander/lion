/*------------------------------------------------------------------------
  ...
  ------- -------- -------- ---------------------------------------------
     2.00 08Sep16  Terry    Refactored original
...
------------------------------------------------------------------------*/
  ...
  for each fabd_det
      ...
      where fabd_domain   = global_domain
        ...
        and fabd_fabk_id  = tBook
      break by fabd_yrper by fabd_faloc_id:
  
    if last-of(fabd_faloc_id) then
    do: 
      run getDepreciationAmounts(global_domain, fabd_fa_id, tBook, fabd_yrper, 
        output vAccumDepBk, output vFirstAssetdep, output accDepr,
        output vVariance, output vReport, output vMessage).

      assign
        costAmt = getBookValue(global_domain, fabd_fa_id, tBook, yearPeriod2Date(global_domain, fabd_yrper, false))
